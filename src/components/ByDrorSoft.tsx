...
export const ByDrorSoft = () => {
    const openTerms = () => {
        const el = document.getElementById('terms-dialog') as (HTMLDialogElement | null);
        if (!el) return;
        // Properly type possible dialog methods instead of using `any`.
        const dialogEl = el as HTMLDialogElement & { showModal?: () => void; show?: () => void };
        if (typeof dialogEl.showModal === 'function') {
            dialogEl.showModal();
        } else if (typeof dialogEl.show === 'function') {
            dialogEl.show();
        } else {
            // Fallback: set open attribute
    ...
    return   <div
        ...
        {/* Terms button - placed before the Drorsoft logo as requested */}
        <button
            onClick={openTerms}
            aria-haspopup="dialog"
            className="terms-button"
            style={{background: 'none', border: 'none', padding: 0, marginRight: 8, color: 'inherit', textDecoration: 'underline', cursor: 'pointer'}}
        >
